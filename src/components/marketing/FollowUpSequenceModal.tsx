...
export default function FollowUpSequenceModal({ isOpen, onClose, editingSequence, onSaved }: FollowUpSequenceModalProps) {
    ...
    const insertVariable = (stepId: string, variable: string) => {
        setSteps(prev =>
            prev.map(s => {
                ...
                return { ...s, message_template: s.message_template + variable };
            })
        );
    };
    ...
    const handleSave = async () => {
        ...
        if (emptySteps.length > 0) {
            toast.error('Todas as etapas precisam ter uma mensagem.');
            return;
        }
        ...
        try {
            ...
            let result;
            if (editingSequence) {
                result = await updateFollowupSequence(editingSequence.id, payload);
            } else {
                result = await createFollowupSequence(payload);
            }

            if (result.success) {
                toast.success(editingSequence ? 'Sequência atualizada!' : 'Sequência criada com sucesso!');
                onSaved();
                onClose();
            } else {
    ...
    const getDelayPreview = (step: StepForm, index: number) => {
        ...
        if (index === 0) return `Enviar após ${step.delay_value} ${label} da inscrição`;
        return `Aguardar ${step.delay_value} ${label} após etapa anterior`;
    };
            ...
            {isLoadingEdit ? (
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-[#404F4F] mb-1.5">
                                Nome da Sequência *
                            </label>
                            <input
                                type="text"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <label className="flex items-center gap-3 px-4 py-2.5 rounded-xl border border-border bg-white cursor-pointer hover:bg-gray-50 transition-all">
                                <input
                                    type="checkbox"
                                    checked={exitOnReply}
                                ...
                                />
                                <span className="text-sm text-[#404F4F]">Remover lead ao responder</span>
                            </label>
                            <p className="text-[10px] text-muted-foreground mt-1">
                                Lead passa para atendimento humano ao responder.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 bg-blue-50/50 rounded-xl px-3 py-2">
                            ...
                            {VARIABLES.map(v => (
                                <span
                                    key={v.key}
                                    className="px-2 py-0.5 rounded-lg bg-blue-100 text-blue-700 text-[10px] font-bold"
                                    title={v.label}
                                >
                                    {v.key}
                                </span>
                                    ...
                                    <div className="bg-gray-50 rounded-2xl border border-border/50 p-4 space-y-3 hover:border-[#404F4F]/20 transition-all">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <div className="w-6 h-6 rounded-lg bg-[#404F4F] text-white flex items-center justify-center text-[10px] font-black">
                                                    {index + 1}
                                                </div>
                                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                                                    {getDelayPreview(step, index)}
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                {index > 0 && (
                                                    <button
                                                        ...
                                                        className="p-1 rounded-md text-muted-foreground hover:bg-white transition-all text-xs"
                                                ...
                                                <button
                                                    ...
                                                    className="p-1 rounded-md text-muted-foreground hover:text-red-600 hover:bg-red-50 transition-all"
                                                    title="Remover etapa"
                                                >
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input
                                                ...
                                                className="w-16 px-2 py-1.5 rounded-lg border border-border bg-white text-sm text-center focus:outline-none focus:ring-2 focus:ring-accent-icon/20"
                                            />
                                            <select
                                                value={step.delay_unit}
                                                onChange={e => updateStep(step.id, 'delay_unit', e.target.value)}
                                                className="px-2 py-1.5 rounded-lg border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-accent-icon/20"
                                            >
                                                {DELAY_UNITS.map(u => (
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-1">
                                                <label className="text-[10px] font-bold text-[#404F4F] uppercase tracking-wider">
                                                    Mensagem
                                                </label>
                                                <div className="flex gap-1">
                                                    {VARIABLES.map(v => (
                                                        <button
                                                            key={v.key}
                                                            onClick={() => insertVariable(step.id, v.key)}
                                            ...
                                            <p className="text-[9px] text-muted-foreground mt-0.5 text-right">
                                                {step.message_template.length} caracteres
                                            </p>
                                        </div>
                                    </div>
                                </div>
