...
import { FileText, Upload, Trash2, Loader2, CheckCircle2, XCircle, AlertCircle, ChevronDown } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="col-span-1 flex flex-col space-y-3">
                    <div className="px-1 space-y-1">
                        <h3 className="text-sm font-bold text-foreground uppercase tracking-widest">Enviar Documento</h3>
                    </div>
                    <div className="bg-white dark:bg-background border border-border/40 rounded-lg p-6 space-y-6 flex-1">

                        <div>
                            <label className="block text-[11px] font-bold text-foreground/80 ml-0.5 mb-2">Tipo do Arquivo</label>
                            <div className="relative">
                                <select
                                    value={docType}
                                    onChange={(e) => setDocType(e.target.value)}
                                    disabled={uploading}
                                    className="appearance-none w-full bg-gray-50 dark:bg-input border border-gray-200 dark:border-border focus:border-primary/50 focus:ring-1 focus:ring-primary/20 rounded-lg px-3 py-2.5 text-xs font-medium text-foreground outline-none transition-all cursor-pointer pr-8"
                                >
                        ...
                        <div>
                            <label className="block text-[11px] font-bold text-foreground/80 ml-0.5 mb-2">Arquivo</label>
                            <label className="border border-border/40 rounded-lg p-5 flex flex-col items-center justify-center gap-1.5 hover:bg-gray-50 dark:hover:bg-muted/30 transition-all cursor-pointer">
                                {uploading ? (
                        ...
                        <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-3 border border-border/30 flex gap-2">
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col space-y-3">
                    <div className="px-1 space-y-1">
                        <h3 className="text-sm font-bold text-foreground uppercase tracking-widest">Documentação Anexa</h3>
                    </div>
                    <div className="bg-white dark:bg-background border border-border/40 rounded-lg p-6 space-y-4 flex-1">
                                    ...
                                    return (
                                        <div key={doc.id} className="flex items-center justify-between py-3 border-b border-border/10 last:border-0 group">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className={`text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${doc.status === 'aprovado' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-400' :
