...
export function NotificationsList({ notifications, setNotifications }: NotificationsListProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-white md:bg-transparent rounded-lg md:rounded-none">
            {/* Header Actions */}
            <div className="flex flex-wrap items-center justify-between p-4 border-b border-gray-100 gap-4 bg-white sticky top-0 z-10">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex gap-2">
                        ...
                        <span className="text-xs px-2 py-1 rounded-full bg-red-50 text-red-600 font-medium whitespace-nowrap">
            ...
            {(selectedIds.length > 0) && (
                <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-end gap-2 animate-in fade-in slide-in-from-top-1">
                    <button
                        onClick={markAsRead}
                ...
                {notifications.length === 0 ? (
                    ...
                    <div className="divide-y divide-gray-50">
                        {notifications.map((notification) => (
                            <div
                                key={notification.id}
                                className={`flex items-start gap-3 p-4 hover:bg-gray-50 transition-colors group ${!notification.read ? 'bg-blue-50/30' : ''
                                    }`}
                            >
                                <button
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between mb-1">
                                        ...
                                        <span className="text-xs text-gray-400 whitespace-nowrap sm:ml-2">{notification.date}</span>
                                    </div>
