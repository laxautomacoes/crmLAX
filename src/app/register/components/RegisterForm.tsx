    ...
    return (
        <form className="space-y-4 md:space-y-5" onSubmit={onSubmit}>
            ...
            <FormInput
                ...
                icon={Mail}
                className={`py-3 md:py-3.5 ${isInvited ? 'bg-muted/50 cursor-not-allowed opacity-70' : ''}`}
            />
            ...
            <div className="flex justify-center items-center mt-4 md:mt-6">
                <span className="text-muted-foreground text-xs md:text-sm font-medium mr-1.5">Já tem uma conta?</span>
                <Link href="/login" className="text-xs md:text-sm font-bold text-foreground hover:opacity-80 transition-opacity">Fazer Login</Link>
            </div>
        </form>
    );
}
