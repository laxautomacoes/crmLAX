    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            {isLoading ? (
                ...
                viewMode === 'grid' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredNotes.map((note) => (
                            <div key={note.id} className="bg-white rounded-2xl border border-border/60 shadow-sm hover:shadow-md transition-all flex flex-col group p-6 space-y-4">
                                ...
                                <div 
                                    className="text-gray-700 text-sm line-clamp-4 leading-relaxed overflow-hidden prose prose-sm max-w-none"
                                    dangerouslySetInnerHTML={{ __html: note.content }}
                                />

                                <div className="space-y-2 pt-2 border-t border-border/40">
                                    {note.assets && (
                                        <div className="flex items-center gap-2 text-[11px] font-semibold text-secondary bg-secondary/5 px-3 py-1.5 rounded-lg border border-secondary/10 w-fit max-w-full">
                                            <Building2 size={12} className="shrink-0" />
                                            <span className="truncate">{note.assets.title}</span>
                                        </div>
                                    )}
                                    {note.leads && (
                                        <div className="flex items-center gap-2 text-[11px] font-semibold text-[#404F4F] bg-muted/50 px-3 py-1.5 rounded-lg border border-border/60 w-fit max-w-full">
                                            <User size={12} className="shrink-0" />
                                            <span className="truncate">{note.leads.contacts?.name || 'Lead'}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="rounded-2xl">
                            <table className="w-full text-left border-separate border-spacing-0">
                                <thead className="bg-muted/50">
                                    <tr>
                                        <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-wider border-b border-border/60 first:rounded-tl-2xl">Data</th>
                                        <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-wider border-b border-border/60">Conteúdo</th>
                                        <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-wider border-b border-border/60">Relacionados</th>
                                        <th className="px-6 py-4 text-xs font-bold text-muted-foreground uppercase tracking-wider text-right border-b border-border/60 last:rounded-tr-2xl">Ações</th>
                                    </tr>
                                </thead>
                                    ...
                                    {filteredNotes.map((note, index) => (
                                        <tr key={note.id} className="hover:bg-muted/30 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 max-w-md">
                                                <div 
                                                    className="text-sm text-gray-700 line-clamp-2 prose prose-sm"
                                                    dangerouslySetInnerHTML={{ __html: note.content }}
                                                />
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-wrap gap-2">
                                                    {note.assets && (
                                                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-secondary bg-secondary/5 px-2 py-1 rounded-md border border-secondary/10">
                                                            <Building2 size={10} />
                                                            {note.assets.title}
                                                        </div>
                                                    )}
                                                    {note.leads && (
                                                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-[#404F4F] bg-muted/50 px-2 py-1 rounded-md border border-border/60">
                                                            <User size={10} />
                                                            {note.leads.contacts?.name || 'Lead'}
                                                        </div>
                                                    )}
                                                </div>
                                            </td>
                ...
                <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border-2 border-dashed border-border/60">
                    <div className="p-6 bg-muted/30 rounded-full mb-4">
                        <FileText size={48} className="text-muted-foreground/40" />
                    </div>
                    <h3 className="text-lg font-bold text-[#404F4F]">Nenhuma nota encontrada</h3>
